...
from PyQt5.QtWidgets import QApplication, QMainWindow, QWidget, QVBoxLayout, QHBoxLayout, QComboBox, QLabel, QPushButton, QGraphicsScene, QGraphicsView, QGraphicsEllipseItem, QGraphicsLineItem, QLineEdit, QFormLayout, QDockWidget
...
class SkillTreeGUI(QMainWindow):
    def __init__(self):
        ...
        # Spacing factors (default values)
        self.spacing_factor = 0.8  # Vertical spacing
        self.horizontal_spacing_factor = 0.8  # Horizontal spacing

        # Starting area marker size (default value)
        self.start_marker_size = 50
        ...
        self.start_marker = None  # Marker for starting area
    ...
    def resizeEvent(self, event):
        ...
        try:
            ...
            self.load_skill_tree_once()  # Reload to adjust for new size
    ...
    def init_ui(self):
        ...
        points_layout.addWidget(self.ascendancy_points_label)
        control_layout.addLayout(points_layout)

        # Node size and spacing controls
        settings_form = QFormLayout()
        settings_form.setLabelAlignment(Qt.AlignRight)

        # Node size fields
        self.node_size_fields = {}
        for node_type, default_size in self.node_sizes.items():
            field = QLineEdit(str(default_size))
            field.setFixedWidth(50)
            field.setStyleSheet("background-color: #333; color: white;")
            field.editingFinished.connect(lambda nt=node_type, f=field: self.update_node_size(nt, f.text()))
            settings_form.addRow(f"{node_type} Size:", field)
            self.node_size_fields[node_type] = field

        # Spacing fields
        self.spacing_field = QLineEdit(str(self.spacing_factor))
        self.spacing_field.setFixedWidth(50)
        self.spacing_field.setStyleSheet("background-color: #333; color: white;")
        self.spacing_field.editingFinished.connect(lambda: self.update_spacing(self.spacing_field.text(), self.horizontal_spacing_field.text()))
        settings_form.addRow("Vertical Spacing:", self.spacing_field)

        self.horizontal_spacing_field = QLineEdit(str(self.horizontal_spacing_factor))
        self.horizontal_spacing_field.setFixedWidth(50)
        self.horizontal_spacing_field.setStyleSheet("background-color: #333; color: white;")
        self.horizontal_spacing_field.editingFinished.connect(lambda: self.update_spacing(self.spacing_field.text(), self.horizontal_spacing_field.text()))
        settings_form.addRow("Horizontal Spacing:", self.horizontal_spacing_field)

        # Start marker size field
        self.start_marker_field = QLineEdit(str(self.start_marker_size))
        self.start_marker_field.setFixedWidth(50)
        self.start_marker_field.setStyleSheet("background-color: #333; color: white;")
        self.start_marker_field.editingFinished.connect(lambda: self.update_start_marker_size(self.start_marker_field.text()))
        settings_form.addRow("Start Marker Size:", self.start_marker_field)
        ...
        self.reset_button.setStyleSheet("background-color: #333; color: white;")
    ...
    def update_node_size(self, node_type: str, value: str):
        """Update the size of nodes of the given type."""
        try:
            new_size = float(value)
            if new_size <= 0:
                raise ValueError("Node size must be positive")
            self.node_sizes[node_type] = new_size
            ...
            for node_id, item in list(self.node_items.items()):
                try:
                    ...
                    if (node["is_root"] and node_type == "Root") or node["type"] == node_type:
                        x, y = item.rect().center().x(), item.rect().center().y()
                        item.setRect(x - new_size / 2, y - new_size / 2, new_size, new_size)
                except Exception as e:
                    logger.error(f"Error updating node size for node {node_id}: {str(e)}")
                    continue
            for (class_name, node_id), item in list(self.ascendancy_node_items.items()):
                try:
                    node = next(n for n in self.skill_tree_data["ascendancy_trees"][class_name]["nodes"] if n["id"] == node_id)
                    if node["type"] == node_type:
                        x, y = item.rect().center().x(), item.rect().center().y()
                        item.setRect(x - new_size / 2, y - new_size / 2, new_size, new_size)
                except Exception as e:
                    logger.error(f"Error updating Ascendancy node size for ({class_name}, {node_id}): {str(e)}")
                    continue
        except Exception as e:
            logger.error(f"Error updating node size for {node_type}: {str(e)}")

    def update_spacing(self, vertical_value: str, horizontal_value: str):
        """Update the spacing between nodes."""
        try:
            new_vertical_spacing = float(vertical_value)
            new_horizontal_spacing = float(horizontal_value)
            if new_vertical_spacing <= 0 or new_horizontal_spacing <= 0:
                raise ValueError("Spacing factors must be positive")
            self.spacing_factor = new_vertical_spacing
            self.horizontal_spacing_factor = new_horizontal_spacing
            self.load_skill_tree_once()
            self.center_on_class(self.class_combo.currentText())
        except Exception as e:
            logger.error(f"Error updating spacing: {str(e)}")

    def update_start_marker_size(self, value: str):
        """Update the size of the starting area marker."""
        try:
            new_size = float(value)
            if new_size <= 0:
                raise ValueError("Start marker size must be positive")
            self.start_marker_size = new_size
            # Update the marker if it exists
            if self.start_marker:
                x, y = self.start_marker.rect().center().x(), self.start_marker.rect().center().y()
                self.start_marker.setRect(x - new_size / 2, y - new_size / 2, new_size, new_size)
        except Exception as e:
            logger.error(f"Error updating start marker size: {str(e)}")
    ...
    def load_skill_tree_once(self):
        ...
        try:
            ...
            if not main_nodes:
                logger.error("No main nodes found in skill tree data")
                return
            ...
            if not x_coords or not y_coords:
                logger.error("No coordinates found in main nodes")
                return
            ...
            x_scale = self.view.width() / x_range * self.horizontal_spacing_factor
            y_scale = self.view.height() / y_range * self.spacing_factor

            # Store bounds for scene rect
            self.scene_x_min = (x_min - 100) * x_scale  # Add padding
            self.scene_x_max = (x_max + 100) * x_scale
            self.scene_y_min = (y_min - 100) * y_scale
            self.scene_y_max = (y_max + 100) * y_scale
            ...
            for conn in main_connections:
                ...
                x1 = (from_node["x"] * x_scale) - (x_min * x_scale) + tree_center_x - (x_range * x_scale) / 2
                y1 = (from_node["y"] * y_scale) - (y_min * y_scale) + tree_center_y - (y_range * y_scale) / 2
                x2 = (to_node["x"] * x_scale) - (x_min * x_scale) + tree_center_x - (x_range * x_scale) / 2
                y2 = (to_node["y"] * y_scale) - (y_min * y_scale) + tree_center_y - (y_range * y_scale) / 2
            ...
            for node in main_nodes:
                x = (node["x"] * x_scale) - (x_min * x_scale) + tree_center_x - (x_range * x_scale) / 2
                y = (node["y"] * y_scale) - (y_min * y_scale) + tree_center_y - (y_range * y_scale) / 2
                node_type = "Root" if node["is_root"] else node["type"]
                size = self.node_sizes[node_type]
                color = QColor("red") if node["is_root"] else QColor("gold") if node["type"] == "Keystone" else QColor("orange") if node["type"] == "Notable" else QColor("purple") if node["type"] == "Mastery" else QColor("gray") if node["type"] == "Small" else QColor("cyan")
            ...
            for class_name in self.skill_tree_data["classes"].keys():
                if class_name in self.skill_tree_data["ascendancy_trees"]:
                    ...
                    ax_coords = [node["x"] for node in ascendancy_nodes] if ascendancy_nodes else [0]
                    ay_coords = [node["y"] for node in ascendancy_nodes] if ascendancy_nodes else [0]
                    ax_min, ax_max = (min(ax_coords), max(ax_coords)) if ax_coords else (0, 1)
                    ay_min, ay_max = (min(ay_coords), max(ay_coords)) if ay_coords else (0, 1)
                    ...
                    for node in ascendancy_nodes:
                        ...
                        size = self.node_sizes[node["type"]]
    ...
    def center_on_class(self, selected_class: str):
        """Center the view on the starting node of the selected class and mark the starting area."""
        try:
            # Get starting nodes for the selected class
            starting_nodes = self.skill_tree_data["classes"][selected_class]["starting_nodes"]
            if not starting_nodes:
                logger.error(f"No starting nodes found for class {selected_class}. Falling back to tree center.")
                # Fall back to the center of the tree
                start_x = (self.scene_x_max + self.scene_x_min) / 2 if hasattr(self, 'scene_x_max') else 0
                start_y = (self.scene_y_max + self.scene_y_min) / 2 if hasattr(self, 'scene_y_max') else 0
            else:
                ...
                if not x_coords or not y_coords:
                    logger.error("No coordinates found in main nodes for centering")
                    start_x = (self.scene_x_max + self.scene_x_min) / 2 if hasattr(self, 'scene_x_max') else 0
                    start_y = (self.scene_y_max + self.scene_y_min) / 2 if hasattr(self, 'scene_y_max') else 0
                else:
                    x_min = min(x_coords)
                    ...
                    start_x = (starting_node["x"] * x_scale) - (x_min * x_scale) + tree_center_x - (x_range * x_scale) / 2
                    start_y = (starting_node["y"] * y_scale) - (y_min * y_scale) + tree_center_y - (y_range * y_scale) / 2

                    # Add a marker for the starting area
                    if self.start_marker:
                        self.scene.removeItem(self.start_marker)
                    self.start_marker = QGraphicsEllipseItem(start_x - self.start_marker_size / 2, start_y - self.start_marker_size / 2, self.start_marker_size, self.start_marker_size)
                    self.start_marker.setBrush(QBrush(QColor(255, 255, 0, 50)))  # Semi-transparent yellow
                    self.start_marker.setPen(QPen(Qt.NoPen))
                    self.start_marker.setZValue(-1)  # Place behind nodes
            ...
            for class_name in self.skill_tree_data["classes"].keys():
                if class_name in self.skill_tree_data["ascendancy_trees"]:
                    ...
                    if self.ascendancy_connection_items:  # Check if dictionary is not empty
                        for (cls, from_id, to_id), line in list(self.ascendancy_connection_items.items()):
                            if cls == class_name:
                                try:
                                    ...
                                    logger.error(f"Error updating Ascendancy connection ({cls}, {from_id}, {to_id}): {str(e)}")
                                    continue

                    # Update Ascendancy nodes
                    if self.ascendancy_node_items:  # Check if dictionary is not empty
                        for (cls, node_id), item in list(self.ascendancy_node_items.items()):
                            if cls == class_name:
                                try:
                                    ...
                                    logger.error(f"Error updating Ascendancy node ({cls}, {node_id}): {str(e)}")
                                    continue
    ...
    def center_on_ascendancy(self):
        ...
        try:
            ...
            if self.ascendancy_connection_items:  # Check if dictionary is not empty
                for (cls, _, _), item in list(self.ascendancy_connection_items.items()):
                    try:
                        item.setVisible(False)
                    except Exception as e:
                        logger.error(f"Error hiding Ascendancy connection ({cls}): {str(e)}")
                        continue
            if self.ascendancy_node_items:  # Check if dictionary is not empty
                for (cls, _), item in list(self.ascendancy_node_items.items()):
                    try:
                        item.setVisible(False)
                    except Exception as e:
                        logger.error(f"Error hiding Ascendancy node ({cls}): {str(e)}")
                        continue

            # Show root nodes if Ascendancy is not selected
            if self.node_items:  # Check if dictionary is not empty
                for node_id, item in list(self.node_items.items()):
                    try:
                        node = next(n for n in self.skill_tree_data["main_tree"]["nodes"] if n["id"] == node_id)
                        if node["is_root"]:
                        ...
                        logger.error(f"Error updating root node visibility ({node_id}): {str(e)}")
                        continue

            # Show Ascendancy tree if selected
            if selected_ascendancy != "No Ascendancy":
                self.ascendancy_points_label.show()
                if self.ascendancy_connection_items:
                    for (cls, _, _), item in list(self.ascendancy_connection_items.items()):
                        if cls == selected_class:
                            try:
                                item.setVisible(True)
                            except Exception as e:
                                logger.error(f"Error showing Ascendancy connection ({cls}): {str(e)}")
                                continue
                if self.ascendancy_node_items:
                    for (cls, node_id), item in list(self.ascendancy_node_items.items()):
                        if cls == selected_class:
                            try:
                                item.setVisible(True)
                            except Exception as e:
                                logger.error(f"Error showing Ascendancy node ({cls}, {node_id}): {str(e)}")
                                continue
            else:
                    ...
                    for (cls, node_id), item in list(self.ascendancy_node_items.items()):
                        if cls == selected_class:
                            try:
                                ...
                                logger.error(f"Error resetting Ascendancy node color ({cls}, {node_id}): {str(e)}")
                                continue
                if self.ascendancy_connection_items:
                    for (cls, _, _), line in list(self.ascendancy_connection_items.items()):
                        if cls == selected_class:
                            try:
                                line.setPen(QPen(QColor("gray"), 1))
                            except Exception as e:
                                logger.error(f"Error resetting Ascendancy connection color ({cls}): {str(e)}")
                                continue
    ...
    def node_clicked(self, node, event):
        ...
        try:
            ...
            if self.allocated_points >= MAX_POINTS:
                logger.debug(f"Cannot allocate node {node['id']}: Maximum points reached")
                return

            # Check if node is reachable
            if not self.can_allocate_node(node["id"]):
                logger.debug(f"Cannot allocate node {node['id']}: Not connected to an allocated node")
                return
            ...
            self.update_points_display()
            logger.debug(f"Node {node['id']} allocated")
        except Exception as e:
    ...
    def ascendancy_node_clicked(self, node, event):
        ...
        try:
            ...
            if self.allocated_ascendancy_points >= MAX_ASCENDANCY_POINTS:
                logger.debug(f"Cannot allocate Ascendancy node {node['id']}: Maximum points reached")
                return

            # Check if node is reachable in Ascendancy tree
            if not self.can_allocate_ascendancy_node(node["id"]):
                logger.debug(f"Cannot allocate Ascendancy node {node['id']}: Not connected to an allocated node")
                return
            ...
            self.update_points_display()
            logger.debug(f"Ascendancy node {node['id']} allocated")
        except Exception as e:
    ...
    def update_connections(self):
        ...
        try:
            if self.connection_items:  # Check if dictionary is not empty
                for (from_id, to_id), line in list(self.connection_items.items()):
                    try:
                        ...
                        logger.error(f"Error updating main tree connection ({from_id}, {to_id}): {str(e)}")
                        continue
        except Exception as e:
            ...
            if self.node_items:
                for node_id, item in list(self.node_items.items()):
                    try:
                        ...
                        logger.error(f"Error resetting node ({node_id}): {str(e)}")
                        continue
            if self.ascendancy_node_items:
                for (cls, node_id), item in list(self.ascendancy_node_items.items()):
                    if cls == self.class_combo.currentText():
                        try:
                            ...
                            logger.error(f"Error resetting Ascendancy node ({cls}, {node_id}): {str(e)}")
                            continue
            if self.connection_items:
                for line in list(self.connection_items.values()):
                    try:
                        line.setPen(QPen(QColor("gray"), 1))
                    except Exception as e:
                ...
                for (cls, _, _), line in list(self.ascendancy_connection_items.items()):
                    if cls == self.class_combo.currentText():
                        try:
                            ...
                            logger.error(f"Error resetting Ascendancy connection ({cls}): {str(e)}")
                            continue
            self.allocated_nodes.clear()
            self.allocated_ascendancy_nodes.clear()
            self.update_points_display()
