...
POB_RAW_BASE = "https://raw.githubusercontent.com/PathOfBuildingCommunity/PathOfBuilding-PoE2/dev"
TAG_MAP = {"401": "0_2"}
...
def fetch_tree(poe_version: str) -> Path:
    ...
    raw_file = RAW_DIR / f"{poe_version}_{folder}_{timestamp}.json"
...
def load_pipeline(conn: sqlite3.Connection, vid: int, data: dict):
    ...
    # Load effects embedded directly on nodes
    load_effects(conn, vid, nodes)

    # Load effects from passive_skills mapping, if present
    count = 0
    for nid_str, node in nodes.items():
        try:
            ...
            continue
        for stat_key in skills_data.get(node.get("skill_id"), {}).get("stats", []):
            conn.execute(EFFECT_INSERT_SQL, (nid, stat_key, 0.0, vid))
            count += 1
    logger.info(f"Also loaded {count} node_effects from passive_skills mapping")
...
if __name__ == "__main__":
    ...
    pf.add_argument("--poe-version", default="401")
    pl = sub.add_parser("load")
    pl.add_argument("--json-file", type=Path, required=True)
    pr = sub.add_parser("run")
    pr.add_argument("--poe-version", default="401")
