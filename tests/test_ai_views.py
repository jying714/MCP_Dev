...
def conn(tmp_path):
    # Copy the real DB into a temp file
...
def test_vw_boss_fire_penetration(conn):
    # At least one boss skill should appear in the fire-penetration view
    ...
    cur = conn.execute("""
        PRAGMA table_info('vw_boss_fire_penetration');
    """)
...
def test_vw_gems_projectile(conn):
    # Expect at least one gem tagged 'projectile'
    cur = conn.execute("SELECT COUNT(*) FROM vw_gems_projectile;")
    count, = cur.fetchone()
    assert count > 0, "Expected >0 rows in vw_gems_projectile"

    # Verify that every returned gem has 'projectile' in its tags definition
    cur = conn.execute("""
        SELECT gem_name 
          FROM vw_gems_projectile 
          LIMIT 5
    """)
